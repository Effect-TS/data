...
describe.concurrent("Inspectable", () => {
  describe.concurrent("toString", () => {
    ...
    it("objects", () => {
      expect(Inspectable.toString({ a: 1 })).toEqual("a: 1")
      expect(Inspectable.toString({ a: 1, b: 2 })).toEqual(`a: 1
b: 2`)
      expect(Inspectable.toString({ a: 1, b: { c: 2 } })).toEqual(`a: 1
b:
  c: 2`)
      expect(Inspectable.toString({ a: undefined })).toEqual("a: undefined")
    })

    it("arrays", () => {
      expect(Inspectable.toString([1, 2, 3])).toEqual(`- 1
- 2
- 3`)
      expect(Inspectable.toString([1, [2, 3], 4])).toEqual(`- 1
-
  - 2
  - 3
- 4`)
