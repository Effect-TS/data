...
describe.concurrent("ImmutableQueue", () => {
  ...
  test("dequeue", () => {
    ...
    assert.isTrue(equals(result1, O.some([1, IQ.make(2)])))
    assert.isTrue(equals(result2, O.some([2, IQ.empty<number>()])))
  ...
  test("head", () => {
    ...
    assert.isTrue(equals(IQ.head(queue), O.some(1)))
  })

  test("tail", () => {
    const queue = IQ.make(1, 2, 3, 4, 5)

    assert.isTrue(equals(IQ.tail(queue), O.some(IQ.make(2, 3, 4, 5))))
  })
  ...
  test("map", () => {
    ...
    assert.isTrue(equals(result, IQ.make(2, 3, 4, 5, 6)))
  })
