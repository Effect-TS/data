...
describe.concurrent("RedBlackTree", () => {
  it("forEach", () => {
    ...
    pipe(
      RedBlackTree.empty<number, string>(number.Ord),
  ...
  it("iterable empty", () => {
    const tree = RedBlackTree.empty<number, string>(number.Ord)
  ...
  it("find Eq/Ord", () => {
    ...
    const ord = pipe(number.Ord, Ord.contramap((key: Key) => key.n))
