 ...
 * @since 1.0.0
 */
export const min = <A>(B: Bounded<A>): Monoid<A> => monoid.fromSemigroup(semigroup.min(B), B.maxBound)
 ...
 * @since 1.0.0
 */
export const max = <A>(B: Bounded<A>): Monoid<A> => monoid.fromSemigroup(semigroup.max(B), B.minBound)
